...
std::string default_data_directory() {
    ...
    return "sdmc:/switch/bowling";
#elif defined(__APPLE__)
    return environment("HOME") + "/Library/Application Support/iPod The Sims Bowling";
#elif defined(_WIN32)
    return environment("APPDATA") + "\\iPod The Sims Bowling";
#else
    const std::string xdg = environment("XDG_DATA_HOME");
    return (xdg.empty() ? environment("HOME") + "/.local/share" : xdg) + "/ipod-bowling";
#endif
}
...
std::string data_directory() {
    std::string directory = environment("BOWLING_DATA_DIR");
    if (directory.empty()) {
