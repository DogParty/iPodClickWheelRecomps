...
// BOWLING_TRACE_FILES=1 prints one line per file operation. The emulator has the same in
...
void trace(const char* what, const std::string& name, uint32_t a, uint32_t b, uint32_t result) {
    static const bool on = std::getenv("BOWLING_TRACE_FILES") != nullptr;
    if (on) {
...
uint32_t open_for_read(const std::string& name) {
    ...
    return register_open(OpenFile{read_host_file(path), 0, path.string(), name, false, false});
}
...
// The one save the store owns, by the name the game opens it with: `savefile.dat` at the
// folder's root (PLAN.md difference 6). The game opens it during its boot with mode 1 and an
// 18 784-byte buffer, and creates it empty on the device; whether that buffer is where a save
// is *loaded to* or *written from* is the open question the Hold'em recomp left too, and until
// it is answered a save is opened to be read whatever mode it names — which is also what the
// recordings contain, since the emulator's write-on-open path is off. A save is read from and
// written to the platform's store rather than the game directory, so a platform that keeps
// saves somewhere else (a console's archive, an app's private storage) reads them back from
// there too.
bool is_save_name(const std::string& name) {
    return lowercase(name) == "savefile.dat";
}
...
bool commit(OpenFile& file) {
    if (file.saved) {
        return platform::save_store().store(file.name, file.bytes);
    }
...
uint32_t async_open(uint32_t open_mode, uint32_t path, uint32_t request) {
    ...
    // A saved game belongs to the platform's store rather than to the game directory, and this
    // game writes one the ordinary way — an open, a transfer, a close. It never calls the store
    // ordinals (#12/#14/#16) that the Mini Golf recomp was built around, so the special case
    // that came with them, which refused to treat a save as writable at all, threw every one of
    // this game's writes away.
    //
    // A write-mode open must still not *truncate*, because the game opens a save with mode 1
    // whether it means to read it or write it, and emptying it on the way to reading it is how
    // a save stops surviving a restart. So an open keeps whatever is already there and the
    // bytes are only replaced when a transfer actually brings some.
    //
    // For a save, that mode says nothing at all about which way the bytes are going to move,
    // because this game does not write a save through this call. It writes one through the store
    // ordinals — `#12` open, `#16` write, `#14` close — and Save and Exit was watched doing
    // exactly that: three of those groups, 26 bytes of `options.sav`, 12 524 of `bowling.sav0` and
    // 88 more, with no transfer to a file anywhere in the run. The only thing a save is ever
    // opened here *for* is to be read back at start-up.
    //
    // So a save is opened to be read whatever mode it names. Believing the mode instead pointed
    // every one of those start-up transfers at the file rather than at the game, and booting the
    // game overwrote the save with the buffer it had meant to fill: 12 333 of 12 524 bytes gone,
    // on the first launch after saving.
    const bool save = is_save_name(name);
    const bool writing = mode == OPEN_MODE_WRITE && !save;
    uint32_t handle = 0;
    if (save) {
        // A save that is not there yet still opens, empty: the game expects to be able to open
        // one before it has ever written it, and decides there is none from the magic words it
        // does not find rather than from the open failing.
        std::vector<uint8_t> saved;
        static_cast<void>(platform::save_store().load(name, saved));
        handle = register_open(OpenFile{std::move(saved), 0, {}, name, writing, true});
    } else {
