...
#
#   tests/frames.sh <case> [path/to/bowling-headless]
#
...
case_name=${1:?usage: tests/frames.sh <case> [bowling-headless]}
binary=${2:-}
...
[ -n "$binary" ] || binary="$here/build/bowling-headless"
...
# would then hide.
BOWLING_SHOT_DIR="$frames_dir/recomp" "$binary" "$game_dir/$GAME_IMAGE_PATH" \
