...
void test_default_is_the_ipod() {
    check(bowling::gfx::render_scale() == 1, "the iPod's own resolution is where this starts");
    check(bowling::gfx::screen_width() == bowling::gfx::SCREEN_WIDTH, "and its own width");
    check(bowling::gfx::screen_height() == bowling::gfx::SCREEN_HEIGHT, "and its own height");
}

void test_the_buffer_grows() {
    bowling::gfx::set_render_scale(3);
    check(bowling::gfx::render_scale() == 3, "the scale is what was asked for");
    check(bowling::gfx::screen_width() == 3 * bowling::gfx::SCREEN_WIDTH,
          "the picture is three times as wide");
    check(bowling::gfx::screen_height() == 3 * bowling::gfx::SCREEN_HEIGHT,
          "and three times as tall");
    check(bowling::gfx::screen_pixels() != nullptr, "and there is a buffer behind it");
}

void test_out_of_range_is_clamped() {
    bowling::gfx::set_render_scale(99);
    check(bowling::gfx::render_scale() == 8,
          "a scale this rasteriser will not draw is clamped down");
    bowling::gfx::set_render_scale(0);
    check(bowling::gfx::render_scale() == 1, "and one below the iPod's is clamped up");
}
...
void test_the_oracle_owns_the_renderer() {
    bowling::gfx::set_render_scale(4);
    bowling::gfx::set_high_resolution_text(true);
    bowling::gfx::set_emulator_graphics(true);
    check(bowling::gfx::render_scale() == 1, "the emulator's graphics are 320x240 and nothing else");
    check(bowling::gfx::screen_width() == bowling::gfx::SCREEN_WIDTH, "so the buffer went back too");
    bowling::gfx::set_emulator_graphics(false);
}
