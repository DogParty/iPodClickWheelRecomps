...
const ManifestEntry GAME_MANIFEST[] = {
    {"l", 21, 0x4b9fd426u},
    {"Executables/SimsBowling_1_1_3002478.bin", 23, 0x50536bfdu},
};
...
fs::path make_source(const fs::path& root) {
    write(root / "l", bowling::gamedata::FIRST_CONTENTS);
    write(root / bowling::gamedata::GAME_IMAGE_PATH, bowling::gamedata::SECOND_CONTENTS);
    return root;
}

void test_installs_from_the_games_folder(const fs::path& scratch) {
    const fs::path source = make_source(scratch / "from-folder" / "1500C");
    const std::string target = (scratch / "installed-folder").string();
    std::string why;
    check(bowling::gamedata::install_from_directory(source.string(), target, why),
          "the game's own folder installs");
    check(bowling::gamedata::verify_installed(target, why), "and then verifies");
    check(fs::exists(fs::path(target) / bowling::gamedata::GAME_IMAGE_PATH),
          "the file in the subdirectory arrived");
}

void test_installs_from_the_folder_above(const fs::path& scratch) {
    // A player who picks the folder that *contains* 1500C means the same thing.
    make_source(scratch / "from-parent" / "1500C");
    const std::string target = (scratch / "installed-parent").string();
    std::string why;
    check(bowling::gamedata::install_from_directory((scratch / "from-parent").string(), target, why),
          "the folder above the game's installs too");
}

void test_refuses_a_damaged_source_before_writing(const fs::path& scratch) {
    const fs::path source = make_source(scratch / "damaged" / "1500C");
...
void test_a_played_folder_still_verifies(const fs::path& scratch) {
    const fs::path source = make_source(scratch / "played" / "1500C");
    const std::string target = (scratch / "installed-played").string();
    std::string why;
    check(bowling::gamedata::install_from_directory(source.string(), target, why), "installs");
    // What the game and this program write into the game's folder afterwards.
    write(fs::path(target) / "bowling.sav0", "a saved game");
    write(fs::path(target) / "settings.txt", "format 1\n");
    check(bowling::gamedata::verify_installed(target, why),
...
void test_an_empty_directory_is_not_the_game(const fs::path& scratch) {
    ...
    std::string why;
    check(!bowling::gamedata::install_from_directory(source.string(),
...
int main() {
    ...
    for (const auto& [contents, entry] :
         {std::pair{bowling::gamedata::FIRST_CONTENTS, bowling::gamedata::GAME_MANIFEST[0]},
          std::pair{bowling::gamedata::SECOND_CONTENTS, bowling::gamedata::GAME_MANIFEST[1]}}) {
    ...
    std::error_code error;
    const fs::path scratch = fs::temp_directory_path() / "bowling-install-test";
