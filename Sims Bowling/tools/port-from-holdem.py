#!/usr/bin/env python3
"""Copy the layers this title has not yet been able to take from the shared core out of the Texas
Hold'em recomp, and record what was copied.
...
Hold'em, not Lost, is the source for every file: its tree is the newest copy of the same layers,
carries the forwarding headers into the shared core, the fixes made since (the path-honouring file
lookup, the wider link-setup window's twelve instructions, the tracing aids), and its `libeapp`
files are the same files as Lost's with more taught to them. Where Sims Bowling's behaviour is
Lost's rather than Hold'em's (the event-list buttons, PLAN.md difference 2), the Lost file is read
and the change made by hand here, with its provenance in the comment.

**Identifiers are rewritten; prose is not.** The C++ namespace and everything built on it
(`holdem` -> `bowling`), the CMake options and include guards (`HOLDEM_` -> `BOWLING_`), the
target and program names and the environment variables are rewritten as the files are copied,
because a namespace called `holdem` in this tree is exactly what the quality rules exist to
prevent. A comment that says "Hold'em issues `#3` with a buffer" is left saying so: it is a true
statement about Hold'em, and rewriting it to name this game would make it a false one. Each
ported file's prose is corrected by hand when the file is adopted for this title —
`grep -rn "Hold'em\\|Texas\\|\\bLost\\b" src tests tools` is the worklist, and it should reach zero
as the port is finished.

    python3 tools/port-from-holdem.py                 # copy every file, write the manifest
    python3 tools/port-from-holdem.py --only PATH...  # copy just these, keeping the rest
    python3 tools/port-from-holdem.py --check         # report drift; copy nothing

`--check` re-hashes the Hold'em originals and this tree's copies and says which files have
...
#
# Not ported, and why:
#   src/game/            Hold'em has none yet. Nothing to inherit; this is the work.
#   src/gamedata/manifest_data.cpp   a table of Hold'em's own files. `tools/manifest.py` is
#                        ported and writes this title's from its own folder.
#   tests/expected/, tests/scripts/   recordings and scripts of another game.
#   tools/port-from-lost.py   this file's ancestor; this file replaces it.
...
# `holdem_foo` and the two prefixed rules have to come first.
REWRITES = [
    (r"\bHOLDEM_", "BOWLING_"),
    (r"\bholdem_", "bowling_"),  # CMake targets and C++ symbols
    (r"\bHOLDEM\b", "BOWLING"),
    (r"\bholdem\b", "bowling"),  # the namespace, the program name, the Linux data directory
    (r"iPod Texas Hold'em", "iPod The Sims Bowling"),  # the per-user data directory (paths.cpp)
    (r"ipod-texas-holdem", "ipod-sims-bowling"),
]
...
def check() -> int:
    ...
    for title, paths in (
        ("changed in Hold'em since the port", drifted_source),
