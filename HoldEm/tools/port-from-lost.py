#!/usr/bin/env python3
"""Copy the layers this title has not yet been able to take from the shared core out of the Lost
recomp, and record what was copied.
...
`reference/PORTED.md` with the source path and SHA-256 of every file at the moment it was taken,
so the drift between the copies is one command to measure.

Lost, not Mini Golf, is the source for every file: its tree is the newer copy of the same layers,
carries the fixes made since (a shutdown crash, the `mrs`/`msr` decoder change, the terminate
vector slot), and its `libeapp` files are strict supersets of Mini Golf's — its file layer already
has both the open-as-load and the deferred-completion models this game needs, where Mini Golf's
carries golf-only bookkeeping. Where Hold'em's behaviour is Mini Golf's rather than Lost's (the
button flags word, PLAN.md difference 3), the Mini Golf file is read and the change made by hand
here, with its provenance in the comment.

**Identifiers are rewritten; prose is not.** The C++ namespace and everything built on it
(`lost` -> `holdem`), the CMake options and include guards (`LOST_` -> `HOLDEM_`), the target and
program names and the environment variables are rewritten as the files are copied, because a
namespace called `lost` in this tree is exactly what the quality rules exist to prevent. A comment
that says "Lost's opens are loads" is left saying so: it is a true statement about Lost, and
rewriting it to name this game would make it a false one. Each ported file's prose is corrected by
hand when the file is adopted for this title — `grep -rn '\\bLost\\b' src tests tools` is the
worklist, and it should reach zero as the port is finished.

    python3 tools/port-from-lost.py                 # copy every file, write the manifest
    python3 tools/port-from-lost.py --only PATH...  # copy just these, keeping the rest
    python3 tools/port-from-lost.py --check         # report drift; copy nothing

`--check` re-hashes the Lost originals and this tree's copies and says which files have diverged
on either side since the port.
"""
...
#
# Not ported, and why:
#   src/game/            Lost's game (only `cheats.{h,cpp}` today). Nothing to inherit; this is
#                        the work.
#   src/gamedata/manifest_data.cpp   a table of Lost's own files. `tools/manifest.py` is ported
#                        and writes this title's from its own folder.
#   tests/expected/, tests/scripts/   recordings and scripts of another game.
#   tests/cheats.sh, tests/save-and-exit.sh   outcome tests of Lost's cheats and Lost's save
#                        files. This title will have its own when it has cheats and saves.
...
#                        same eight lines in every tree. When a file moves to the core, its
...
# `lost_foo` and the two prefixed rules have to come first.
REWRITES = [
    (r"\bLOST_", "HOLDEM_"),
    (r"\blost_", "holdem_"),  # CMake targets and C++ symbols
    (r"\bLOST\b", "HOLDEM"),
    (r"\blost\b", "holdem"),  # the namespace, the program name, the Linux data directory
    (r"iPod Lost", "iPod Texas Hold'em"),  # the per-user data directory (src/platform/paths.cpp)
    (r"ipod-lost", "ipod-texas-holdem"),
]
...
def check() -> int:
    ...
    for title, paths in (
        ("changed in Lost since the port", drifted_source),
