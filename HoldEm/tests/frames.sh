...
#
#   tests/frames.sh <case> [path/to/holdem-headless]
#
...
case_name=${1:?usage: tests/frames.sh <case> [holdem-headless]}
binary=${2:-}
...
[ -n "$binary" ] || binary="$here/build/holdem-headless"
...
# would then hide.
HOLDEM_SHOT_DIR="$frames_dir/recomp" "$binary" "$game_dir/$GAME_IMAGE_PATH" \
