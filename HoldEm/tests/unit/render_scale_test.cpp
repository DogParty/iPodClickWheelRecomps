...
void test_default_is_the_ipod() {
    check(holdem::gfx::render_scale() == 1, "the iPod's own resolution is where this starts");
    check(holdem::gfx::screen_width() == holdem::gfx::SCREEN_WIDTH, "and its own width");
    check(holdem::gfx::screen_height() == holdem::gfx::SCREEN_HEIGHT, "and its own height");
}

void test_the_buffer_grows() {
    holdem::gfx::set_render_scale(3);
    check(holdem::gfx::render_scale() == 3, "the scale is what was asked for");
    check(holdem::gfx::screen_width() == 3 * holdem::gfx::SCREEN_WIDTH,
          "the picture is three times as wide");
    check(holdem::gfx::screen_height() == 3 * holdem::gfx::SCREEN_HEIGHT,
          "and three times as tall");
    check(holdem::gfx::screen_pixels() != nullptr, "and there is a buffer behind it");
}

void test_out_of_range_is_clamped() {
    holdem::gfx::set_render_scale(99);
    check(holdem::gfx::render_scale() == 8,
          "a scale this rasteriser will not draw is clamped down");
    holdem::gfx::set_render_scale(0);
    check(holdem::gfx::render_scale() == 1, "and one below the iPod's is clamped up");
}
...
void test_the_oracle_owns_the_renderer() {
    holdem::gfx::set_render_scale(4);
    holdem::gfx::set_high_resolution_text(true);
    holdem::gfx::set_emulator_graphics(true);
    check(holdem::gfx::render_scale() == 1, "the emulator's graphics are 320x240 and nothing else");
    check(holdem::gfx::screen_width() == holdem::gfx::SCREEN_WIDTH, "so the buffer went back too");
    holdem::gfx::set_emulator_graphics(false);
}
