...
//     `options.sav` (26 bytes), `holdem.sav0` (12 524) and one more of 88, with no transfer to any
...
//
//   * a name like `holdem.sav0` is a save. The suffix carries the slot number, so a plain match on
...
constexpr uint32_t REQUEST = holdem::RAM_BASE + 0x1000;
constexpr uint32_t PATH = holdem::RAM_BASE + 0x1100;
constexpr uint32_t BUFFER = holdem::RAM_BASE + 0x1200;
constexpr uint32_t RECORD = holdem::RAM_BASE + 0x1800;
...
void put_string(uint32_t address, const std::string& text) {
    for (size_t i = 0; i < text.size(); ++i) {
        holdem::st8(address + static_cast<uint32_t>(i), static_cast<uint8_t>(text[i]));
    }
    holdem::st8(address + static_cast<uint32_t>(text.size()), 0);
}

void put_bytes(uint32_t address, const std::string& bytes) {
    for (size_t i = 0; i < bytes.size(); ++i) {
        holdem::st8(address + static_cast<uint32_t>(i), static_cast<uint8_t>(bytes[i]));
    }
}
...
std::string take_bytes(uint32_t address, size_t count) {
    ...
    for (size_t i = 0; i < count; ++i) {
        out[i] = static_cast<char>(holdem::ld8(address + static_cast<uint32_t>(i)));
    }
    return out;
}
...
void begin_request(const std::string& name, uint32_t buffer, uint32_t length) {
    for (uint32_t offset = 0; offset < 0x40; offset += 4) {
        holdem::st32(REQUEST + offset, 0);
    }
    put_string(PATH, name);
    holdem::st32(REQUEST + REQUEST_FILE_OBJECT, holdem::RAM_BASE + 0x2000);
    holdem::st32(REQUEST + REQUEST_BUFFER, buffer);
    holdem::st32(REQUEST + REQUEST_LENGTH, length);
    holdem::st8(REQUEST + REQUEST_OPERATION, static_cast<uint8_t>(OPERATION_TRANSFER));
}
...
void save(const std::string& name, const std::string& contents) {
    put_string(PATH, name);
    holdem::st32(RECORD + STORE_RECORD_HANDLE, 0);
    check(holdem::storage::store_open(1, PATH, RECORD) == STORE_OK, "the store opens the save");
    const uint32_t handle = holdem::ld32(RECORD + STORE_RECORD_HANDLE);
    check(handle != 0, "and leaves a handle in the record");
    put_bytes(BUFFER, contents);
    check(holdem::storage::store_write(handle, BUFFER, static_cast<uint32_t>(contents.size())) ==
              STORE_OK,
          "the whole save goes over in one call");
    check(holdem::storage::store_close(handle) == STORE_OK, "and closing it commits");
}
...
std::string load(const std::string& name, size_t expected) {
    ...
    check(holdem::storage::open(OPEN_WRITE, PATH, 0, REQUEST) != 0, "the save opens");
    ...
    const uint32_t read = holdem::storage::perform(REQUEST);
    check(holdem::storage::close(REQUEST) != 0, "and closes again");
...
void test_loading_does_not_empty_it() {
    ...
    save("holdem.sav1", contents);
    check(load("holdem.sav1", contents.size()) == contents, "it loads once");
    check(load("holdem.sav1", contents.size()) == contents, "and loading it did not consume it");
    ...
    begin_request("holdem.sav1", 0, 0);
    check(holdem::storage::open(OPEN_WRITE, PATH, 0, REQUEST) != 0, "it opens once more");
    check(holdem::storage::close(REQUEST) != 0, "and closes with nothing transferred");
    check(load("holdem.sav1", contents.size()) == contents,
...
void test_the_store_is_where_it_goes() {
    ...
    check(holdem::platform::save_store().load("options.sav", bytes), "the store has it");
...
int main() {
    holdem::guest_memory_init();
    holdem::platform::set_save_store(nullptr);  // the in-memory default: nothing reaches the disk
    ...
    test_a_saved_game_survives("holdem.sav0");
