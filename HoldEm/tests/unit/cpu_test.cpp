...
void check_shift(holdem::Shifted got, uint32_t value, bool carry, const char* what) {
    check(got.value == value && got.carry == carry, what);
}

void test_shift_imm() {
    using holdem::Shift;
    using holdem::shift_imm;
...
void test_add_sub_flags() {
    ...
    check(holdem::add_flags(cpu, 0x7FFF'FFFFu, 1u) == 0x8000'0000u,
          "add result wraps into the sign bit");
    check(cpu.n && !cpu.z && !cpu.c && cpu.v, "0x7fffffff + 1: N V set, C clear");

    check(holdem::add_flags(cpu, 0xFFFF'FFFFu, 1u) == 0u, "add result wraps to zero");
    check(!cpu.n && cpu.z && cpu.c && !cpu.v, "0xffffffff + 1: Z C set, V clear");

    check(holdem::sub_flags(cpu, 0u, 1u) == 0xFFFF'FFFFu, "0 - 1 wraps");
    check(cpu.n && !cpu.z && !cpu.c && !cpu.v, "0 - 1: N set, borrow clears C");

    check(holdem::sub_flags(cpu, 5u, 5u) == 0u, "5 - 5 is zero");
    check(!cpu.n && cpu.z && cpu.c && !cpu.v, "equal compare: Z and C set");

    check(holdem::sub_flags(cpu, 0x8000'0000u, 1u) == 0x7FFF'FFFFu, "INT_MIN - 1 wraps");
    check(!cpu.n && !cpu.z && cpu.c && cpu.v, "INT_MIN - 1: signed overflow, no borrow");

    cpu.c = false;
    check(holdem::sbc_flags(cpu, 10u, 3u) == 6u, "SBC subtracts the borrow");
    cpu.c = true;
    check(holdem::adc_flags(cpu, 10u, 3u) == 14u, "ADC adds the carry");
}

void test_conditions() {
    holdem::Cpu cpu;
    holdem::sub_flags(cpu, 3u, 5u);  // 3 < 5, signed and unsigned
    check(holdem::cond_lt(cpu) && holdem::cond_cc(cpu) && holdem::cond_ls(cpu),
          "3 cmp 5: lt, cc (lo), ls");
    check(!holdem::cond_ge(cpu) && !holdem::cond_hi(cpu) && holdem::cond_ne(cpu),
          "3 cmp 5: not ge, not hi, ne");
    holdem::sub_flags(cpu, 0xFFFF'FFFFu, 1u);  // -1 cmp 1 signed; huge cmp 1 unsigned
    check(holdem::cond_lt(cpu) && holdem::cond_hi(cpu), "-1 cmp 1: signed lt, unsigned hi");
}

void test_multiply_long() {
    holdem::Cpu cpu;
    holdem::smull(cpu, 0, 1, 0xFFFF'FFFFu, 2u, true);  // -1 * 2 = -2
    check(cpu.r[0] == 0xFFFF'FFFEu && cpu.r[1] == 0xFFFF'FFFFu && cpu.n && !cpu.z, "smull -1 * 2");
    holdem::umull(cpu, 2, 3, 0xFFFF'FFFFu, 2u, false);
    check(cpu.r[2] == 0xFFFF'FFFEu && cpu.r[3] == 1u, "umull 0xffffffff * 2");
    holdem::umlal(cpu, 2, 3, 1u, 1u, false);
...
void test_status_register() {
    ...
    check((saved & 0xFFu) == holdem::CPSR_MODE_BITS, "mrs reports supervisor mode, interrupts off");
    ...
    check(cpu.cpsr() == (0x0300'0000u | holdem::CPSR_MODE_BITS),
