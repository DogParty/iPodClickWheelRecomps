...
// Reads a zip that contains the game's folder (at any depth: "33333/..." or the files at the
...
// picks `33333` and one who picks the folder containing it both get what they meant.
[[nodiscard]] bool install_from_directory(const std::string& source_dir,
                                          const std::string& game_dir, std::string& why);

// The game directory to run from: `<data_dir>/33333` once it verifies. Until it does, asks the
