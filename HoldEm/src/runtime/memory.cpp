...
//
// -DHOLDEM_REGION_MEMORY builds the console's model on a desktop, which is the only way to run
// the oracle against it: the tests cannot be run on the console itself.
#if defined(HOLDEM_REGION_MEMORY)
// The regions, chosen deliberately.
#elif defined(_WIN32)
#define HOLDEM_FLAT_GUEST_MEMORY 1
#include <windows.h>
#elif defined(__unix__) || defined(__APPLE__)
#define HOLDEM_FLAT_GUEST_MEMORY 1
#include <sys/mman.h>
#if defined(__GLIBC__) || defined(__APPLE__)
#define HOLDEM_HAVE_BACKTRACE 1
#include <execinfo.h>
#endif
#endif
...
#if defined(HOLDEM_FLAT_GUEST_MEMORY)
...
// HOLDEM_WATCH_VALUE narrows the watch to stores of one value, and the watch only fires while
...
// HOLDEM_WATCH_LOG=1 turns the watch into a log: every store is printed and the run goes on,
...
void watch_hit(uint32_t address, uint32_t value) {
    ...
    std::fflush(stdout);
    std::fprintf(stderr, "watch: store of %08x at %08x\n", value, address);
#if defined(HOLDEM_HAVE_BACKTRACE)
    void* frames[24];
...
void guest_memory_init() {
    if (const char* watch = std::getenv("HOLDEM_WATCH")) {
        watch_address = static_cast<uint32_t>(std::strtoul(watch, nullptr, 16));
    }
    watch_logs = std::getenv("HOLDEM_WATCH_LOG") != nullptr;
    if (const char* value = std::getenv("HOLDEM_WATCH_VALUE")) {
