...
// depend on it.
//
//   holdem          [--script=FILE] [--fps=N]
//   holdem-headless <image.bin> --gamedir=DIR [--script=FILE] [--call-log=FILE] [--frames=N]
//
...
Options parse_options(int argc, char** argv) {
    Options options;
    options.program_name = argc > 0 ? argv[0] : "holdem";
...
//
// The game keeps a word of button flags and tests its bits every frame — the same model Mini
// Golf uses, which is no surprise: the two were built by the same SDK in the same week (build
// 2563291 against 2563296). Bits 0x01..0x10 are the five buttons; the press sets the bit and the
// next frame clears it, so one frame with the bit set is one press. The word's address is what
// the emulator's `find_flags_word` derives from the `bic r0,r0,#0x60` signature in the image and
// reports at start-up ("button flags word at 0x180597a8"); reference/eapp-loader/play.rs,
// `press_button`. Mini Golf's is at 0x18037a0c — every title puts it somewhere different.
//
...
constexpr uint32_t BUTTON_FLAGS_ADDRESS = 0x1805'97a8u;
...
// The reasons this game understands, and the one it must see first.
//
// Its tick at 0x18008dec dispatches on the byte through a seven-way table at 0x18008e3c:
// 0 = one-time boot, 1 = run a frame, 2 and 6 = idle, 3/4/5 = lifecycle. The byte does a second
// job as well: the game only *registers* the context as its state object while it reads 0 —
//
//     0x18004988  ldrb r0,[r0,#0] / cmp r0,#0 / bleq 0x180057f8
//
// Seeded to the 5 that Mini Golf's pump uses, the registration never happens, `[0x180595d4]`
// stays null, every later tick reads its dispatch value from address 0, and the game re-runs its
// boot case forever; the second boot builds the table sprites before their textures have loaded
// and dies in a divide by zero. So the init call sees 0 and every frame after it sees 1, which is
// what the emulator's `defaults_for` gives this title (`--ctx-seed=0 --frame-reason=1`;
// reference/eapp-loader/play.rs).
//
// What the game answers at `ctx+0x100`, and whether it ever asks to be suspended the way Lost
// does, is not established: nothing reads the answer yet, and every frame is reason 1. A title
// that wanted to put itself away would find this loop deaf to it — that is listed in PLAN.md,
// and it is the first thing to look at when a Menu hold or a Save and Exit does nothing.
constexpr uint8_t CONTEXT_REASON_BOOT = 0;   // what the init vectors are called with
constexpr uint8_t CONTEXT_REASON_FRAME = 1;  // every frame
...
void save_screenshot(unsigned frame) {
    ...
    // HOLDEM_SHOT_DIR chooses where they go; the default suits a run from the project root.
    const char* directory = std::getenv("HOLDEM_SHOT_DIR");
    char path[256];
    std::snprintf(path, sizeof path, "%s/shot-%02u.ppm", directory != nullptr ? directory : "build",
                  shot_number++);
    std::FILE* file = std::fopen(path, "wb");
    if (file == nullptr) {
        std::printf("screenshot frame %u -> cannot write %s (set HOLDEM_SHOT_DIR)  fnv1a %08x\n",
                    frame, path, hash);
        return;
    }
    std::fprintf(file, "P6\n%u %u\n255\n", width, height);
...
int run(Options options) {
    ...
    std::unique_ptr<platform::Platform> host =
        platform::create_platform("Texas Hold'em", options.frames_per_second);
    ...
    st8(context, CONTEXT_REASON_BOOT);
    ...
    // The emulator takes a sixteen-byte block from the game's heap after initialisation, for the
    // event-list node the titles without a flags word are given their buttons in. This game never
    // reads it, but the block is taken all the same, at the same moment: the addresses of
    // everything the game allocates afterwards depend on it, and the oracle compares them.
    constexpr uint32_t EVENT_NODE_SIZE = 0x10;
    (void)eapp::heap().alloc(EVENT_NODE_SIZE);
    ...
    // Whether this game re-points every attribute before every draw (Lost does; Mini Golf points
    // once and draws many times) decides how the shared rasteriser reads a draw's attributes.
    // Not yet measured for this title — the enable flags are the conservative default, and the
    // picture oracle (tests/frames.sh) is what will say whether that is right. See
    // gfx::set_attributes_repointed_per_draw and ../common/README.md.
    ...
    for (unsigned frame = 0;
        ...
        // Why the firmware is calling: an ordinary frame, every time. See CONTEXT_REASON_* above.
        st8(context, CONTEXT_REASON_FRAME);
...
int main(int argc, char** argv) {
    ...
    holdem::platform::windows_console_begin("Texas Hold'em");
    return holdem::run(holdem::parse_options(argc, argv));
}
