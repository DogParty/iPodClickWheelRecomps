...
// HOLDEM_TRACE_FILES=1 prints one line per file operation. The emulator has the same in
...
void trace(const char* what, const std::string& name, uint32_t a, uint32_t b, uint32_t result) {
    static const bool on = std::getenv("HOLDEM_TRACE_FILES") != nullptr;
    if (on) {
...
// `holdem.sav0`, `holdem.sav1`, … — this game numbers its slots into the suffix, which is why this
...
uint32_t async_open(uint32_t open_mode, uint32_t path, uint32_t request) {
    ...
    // exactly that: three of those groups, 26 bytes of `options.sav`, 12 524 of `holdem.sav0` and
