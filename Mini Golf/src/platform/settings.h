...
// resolved edges wherever the game's geometry is transformed or scaled.
//
// These two are what a settings UI offers and what a saved file is clamped to. The renderer
// clamps again and has the last word.
...
struct Settings {
    ...
    // advances a sixtieth of a second a frame whatever the pace (libeapp/misc.cpp), so 60 is the
    // speed its timers were written for and 30 is that at half speed — which is the default, by
    // choice: it is the pace the game is played at.
