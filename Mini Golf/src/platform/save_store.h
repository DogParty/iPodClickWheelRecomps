...
//
// The definitions live in `recomps/common/src/ipod/platform/save_store.h` and are compiled once for every title.
// This header exists so that nothing else in this tree has to know that: the include path and the
// qualified names callers already write are unchanged, and `minigolf::platform` gains the names by
