...
//
// The iPod had one wheel and five buttons, and this is that set: two directions of the wheel and
// the five buttons, named as a player would name them. Which key, button or gesture performs one
// is the platform's business — a binding here is a plain `InputCode`, and only the platform that
// produced it knows what it means (an SDL keycode, an Android key code, a gamepad button).
//
...
// The seven things a player can do. `SwipeLeft`/`SwipeRight` turn the wheel; the rest are the
// buttons around and inside it.
...
constexpr unsigned ACTION_COUNT = 7;

// Three inputs may perform each action, because one name for a thing is often not enough: the
// menus are a vertical list, so a player reaches for the up and down arrows, while the wheel
// they stand for turned left and right — and a machine with a gamepad plugged in has a third
// name for the same thing again, which should not cost the keyboard one of its two. Slot 0 is
// the one a settings window shows first.
constexpr unsigned BINDING_SLOTS = 3;
