// The settings window: three tabs — General, Input, Graphics.
//
...
//
// Other platforms get an empty stub (settings_window_stub.cpp). Everything that is not the
// window itself — the actions, the defaults, the "one input does one thing" rule, the file — is
// in platform/input_bindings.h, so a further platform's settings UI only has to name keys and
// offer them.
#pragma once
