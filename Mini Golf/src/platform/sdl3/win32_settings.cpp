...
//
// The same three tabs as the Cocoa window, made of the ordinary controls — a tab
...
constexpr int WINDOW_WIDTH = 540, WINDOW_HEIGHT = 400;
...
constexpr int ID_TABS = 100, ID_FRAME_RATE = 101, ID_SHOW_RATE = 102, ID_RENDER_SCALE = 103,
              ID_SCALING = 104, ID_PIXEL_PERFECT = 105, ID_DEFAULTS = 106;
constexpr int ID_KEY_BASE = 200;

constexpr unsigned TAB_COUNT = 3;
constexpr unsigned TAB_GENERAL = 0, TAB_INPUT = 1, TAB_GRAPHICS = 2;
...
private:
    ...
    void build_controls() {
        ...
        const wchar_t* titles[TAB_COUNT] = {L"General", L"Input", L"Graphics"};
    ...
    void build_graphics(int pane_width) {
        ...
        label("The game still computes everything in 320×240 and its sprites are still "
              "enlarged as whole blocks; what gets finer is where an edge lands, wherever the "
              "course is transformed or scaled. Every step costs its square in work — the "
              "rasteriser is software, though it is drawn on every core — so raise it until "
              "the frame rate stops keeping up.",
        ...
        label("The game draws 320×240 pixels and no more, so this is about the last step "
              "only. Sharp enlarges by whole blocks and then softens the fraction left over; "
              "Nearest leaves hard blocks of uneven size; Smooth blurs the lot.",
        ...
        label("Every pixel exactly the same size, at the cost of a border wherever the window "
              "is not a whole multiple of 320×240.",
