...
//
// -DLOST_REGION_MEMORY builds the console's model on a desktop, which is the only way to run
// the oracle against it: the tests cannot be run on the console itself.
#if defined(LOST_REGION_MEMORY)
// The regions, chosen deliberately.
#elif defined(_WIN32)
#define LOST_FLAT_GUEST_MEMORY 1
#include <windows.h>
#elif defined(__unix__) || defined(__APPLE__)
#define LOST_FLAT_GUEST_MEMORY 1
#include <sys/mman.h>
#if defined(__GLIBC__) || defined(__APPLE__)
#define LOST_HAVE_BACKTRACE 1
#include <execinfo.h>
#endif
#endif
...
#if defined(LOST_FLAT_GUEST_MEMORY)
...
#if defined(LOST_HAVE_BACKTRACE)
    void* frames[24];
...
void guest_memory_init() {
    if (const char* watch = std::getenv("LOST_WATCH")) {
