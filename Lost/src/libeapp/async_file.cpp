...
// LOST_TRACE_FILES=1 prints one line per file operation. The emulator has the same in
...
void trace(const char* what, const std::string& name, uint32_t a, uint32_t b, uint32_t result) {
    static const bool on = std::getenv("LOST_TRACE_FILES") != nullptr;
    if (on) {
...
// The saves the store owns, by the names the game opens them with: `options.sav`, and
// `lost.sav0`, `lost.sav1`, … — this game numbers its slots into the suffix, which is why this
// is not a plain comparison and why the Mini Golf recomp's version of it missed every slot but
// the options. A save is read from and written to the platform's store rather than the game
// directory, so a platform that keeps saves somewhere else (a console's archive, an app's
// private storage) reads them back from there too.
bool is_save_name(const std::string& name) {
    const size_t dot = name.rfind(".sav");
    if (dot == std::string::npos) {
        return false;
    }
    return std::all_of(name.begin() + static_cast<long>(dot) + 4, name.end(),
...
uint32_t async_open(uint32_t open_mode, uint32_t path, uint32_t request) {
    ...
    // exactly that: three of those groups, 26 bytes of `options.sav`, 12 524 of `lost.sav0` and
...
uint32_t async_perform(uint32_t request) {
    ...
    if (handle == 0) {
        ...
        got = write_from_guest(handle, buffer, length);
        const OpenFile* file = file_for(handle);
...
using namespace lost::eapp;  // NOLINT(google-build-using-namespace): one file, by design
