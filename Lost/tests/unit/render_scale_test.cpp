...
void test_default_is_the_ipod() {
    check(lost::gfx::render_scale() == 1, "the iPod's own resolution is where this starts");
    check(lost::gfx::screen_width() == lost::gfx::SCREEN_WIDTH, "and its own width");
    check(lost::gfx::screen_height() == lost::gfx::SCREEN_HEIGHT, "and its own height");
}

void test_the_buffer_grows() {
    lost::gfx::set_render_scale(3);
    check(lost::gfx::render_scale() == 3, "the scale is what was asked for");
    check(lost::gfx::screen_width() == 3 * lost::gfx::SCREEN_WIDTH,
          "the picture is three times as wide");
    check(lost::gfx::screen_height() == 3 * lost::gfx::SCREEN_HEIGHT, "and three times as tall");
    check(lost::gfx::screen_pixels() != nullptr, "and there is a buffer behind it");
}

void test_out_of_range_is_clamped() {
    lost::gfx::set_render_scale(99);
    check(lost::gfx::render_scale() == 8, "a scale this rasteriser will not draw is clamped down");
    lost::gfx::set_render_scale(0);
    check(lost::gfx::render_scale() == 1, "and one below the iPod's is clamped up");
}
...
void test_the_oracle_owns_the_renderer() {
    lost::gfx::set_render_scale(4);
    lost::gfx::set_high_resolution_text(true);
    lost::gfx::set_emulator_graphics(true);
    check(lost::gfx::render_scale() == 1, "the emulator's graphics are 320x240 and nothing else");
    check(lost::gfx::screen_width() == lost::gfx::SCREEN_WIDTH, "so the buffer went back too");
    lost::gfx::set_emulator_graphics(false);
}
