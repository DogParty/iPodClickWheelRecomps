...
const ManifestEntry GAME_MANIFEST[] = {
    {"l", 21, 0x4b9fd426u},
    {"Executables/Lost_1_1_2917525.bin", 23, 0x50536bfdu},
};
...
fs::path make_source(const fs::path& root) {
    write(root / "l", lost::gamedata::FIRST_CONTENTS);
    write(root / lost::gamedata::GAME_IMAGE_PATH, lost::gamedata::SECOND_CONTENTS);
    return root;
}

void test_installs_from_the_games_folder(const fs::path& scratch) {
    const fs::path source = make_source(scratch / "from-folder" / "1B200");
    const std::string target = (scratch / "installed-folder").string();
    std::string why;
    check(lost::gamedata::install_from_directory(source.string(), target, why),
          "the game's own folder installs");
    check(lost::gamedata::verify_installed(target, why), "and then verifies");
    check(fs::exists(fs::path(target) / lost::gamedata::GAME_IMAGE_PATH),
          "the file in the subdirectory arrived");
}

void test_installs_from_the_folder_above(const fs::path& scratch) {
    // A player who picks the folder that *contains* 1B200 means the same thing.
    make_source(scratch / "from-parent" / "1B200");
    const std::string target = (scratch / "installed-parent").string();
    std::string why;
    check(lost::gamedata::install_from_directory((scratch / "from-parent").string(), target, why),
          "the folder above the game's installs too");
}

void test_refuses_a_damaged_source_before_writing(const fs::path& scratch) {
    const fs::path source = make_source(scratch / "damaged" / "1B200");
...
void test_a_played_folder_still_verifies(const fs::path& scratch) {
    const fs::path source = make_source(scratch / "played" / "1B200");
    const std::string target = (scratch / "installed-played").string();
    std::string why;
    check(lost::gamedata::install_from_directory(source.string(), target, why), "installs");
    // What the game and this program write into the game's folder afterwards.
    write(fs::path(target) / "lost.sav0", "a saved game");
    write(fs::path(target) / "settings.txt", "format 1\n");
    check(lost::gamedata::verify_installed(target, why),
...
void test_an_empty_directory_is_not_the_game(const fs::path& scratch) {
    ...
    std::string why;
    check(!lost::gamedata::install_from_directory(source.string(),
...
int main() {
    ...
    for (const auto& [contents, entry] :
         {std::pair{lost::gamedata::FIRST_CONTENTS, lost::gamedata::GAME_MANIFEST[0]},
          std::pair{lost::gamedata::SECOND_CONTENTS, lost::gamedata::GAME_MANIFEST[1]}}) {
    ...
    std::error_code error;
    const fs::path scratch = fs::temp_directory_path() / "lost-install-test";
