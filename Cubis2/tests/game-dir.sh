...
GAME_IMAGE_PATH=Executables/Cubis2_1_1_2563292.bin
...
reference_game_dir() {
    here=$(project_root)
    : "${GAME_DIR:=$here/../../../20 iPod games/Games_RO/99999}"
...
# The files the game writes into its folder, which are not shipped: `cubissave.dat` (21 760
# bytes) and `cubisgame.dat` (9 264 bytes), both at the root and both opened for writing during
# the boot (PLAN.md difference 3). "Not shipped" is not an inference here — the game's own
# `Manifest.plist` lists the 114 files Apple sent and names neither. The reference folder this
# repository was developed against holds both from an earlier emulator session, and a case that
# found them would not be a first boot; every private copy starts without them.
strip_written_files() {
    rm -f "$1/cubisgame.dat" "$1/cubissave.dat"
    return 0
}
