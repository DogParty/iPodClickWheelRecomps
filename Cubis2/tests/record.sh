...
#
# A binary named `Cubis2*` matches no arm of the emulator's defaults table and takes its last one
# (`_ => d(true, None, None, None)`, `defaults_for`, play.rs), which is the load-on-open, the seed
# of 5 and the asynchronous file model above; they are spelled out so that the case does not
# depend on a table in another tree. Buttons go by the flags word the emulator finds in the image
# (`button flags word at 0x180a9db0`); there is no flag for it to spell out.
#
# The emulator this records with is the pin under tools/oracle-emulator/, which carries the
# constant-colour fix (reference/MANIFEST.md). A recording made with a build that does not is
# identical call for call — the register is a rendering decision and never reaches the log — but
# every screenshot differs, so the picture oracle must not be run across that line.
#
