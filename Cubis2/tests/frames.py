...
def compare(expected: Path, actual: Path, max_delta: int) -> tuple[float, int]:
    ...
    if (width, height) != (actual_width, actual_height):
        sys.exit(
            f"frames.py: {expected.name} is {width}x{height}, {actual.name} is "
...
def main() -> int:
    ...
    parser.add_argument(
        "--max-differing",
        type=float,
        default=DEFAULT_MAX_DIFFERING_PERCENT,
    ...
    if not expected or len(expected) != len(actual):
        sys.exit(
            f"frames.py: {len(expected)} emulator frame(s) against {len(actual)} recomp "
    ...
    for left, right in zip(expected, actual):
        ...
        if percent > args.max_differing:
            failures += 1
            print(
                f"  {right.name}: {percent:.2f}% of pixels differ by more than "
    ...
    if failures:
        ...
        return 1
    print(
        f"{len(actual)} frames agree with the emulator "
