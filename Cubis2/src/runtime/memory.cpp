...
#if defined(CUBIS_REGION_MEMORY)
// The regions, chosen deliberately.
#elif defined(_WIN32)
#define CUBIS_FLAT_GUEST_MEMORY 1
#include <windows.h>
#elif defined(__unix__) || defined(__APPLE__)
#define CUBIS_FLAT_GUEST_MEMORY 1
#include <sys/mman.h>
#if defined(__GLIBC__) || defined(__APPLE__)
#define CUBIS_HAVE_BACKTRACE 1
#include <execinfo.h>
#endif
#endif
...
#if defined(CUBIS_FLAT_GUEST_MEMORY)
...
// CUBIS_WATCH_VALUE narrows the watch to stores of one value, and the watch only fires while
...
// CUBIS_WATCH_LOG=1 turns the watch into a log: every store is printed and the run goes on,
...
void watch_hit(uint32_t address, uint32_t value) {
    ...
    std::fflush(stdout);
    std::fprintf(stderr, "watch: store of %08x at %08x\n", value, address);
#if defined(CUBIS_HAVE_BACKTRACE)
    void* frames[24];
...
void guest_memory_init() {
    if (const char* watch = std::getenv("CUBIS_WATCH")) {
        watch_address = static_cast<uint32_t>(std::strtoul(watch, nullptr, 16));
    }
    watch_logs = std::getenv("CUBIS_WATCH_LOG") != nullptr;
    if (const char* value = std::getenv("CUBIS_WATCH_VALUE")) {
...
// a write goes nowhere, and either is counted and said once at exit. CUBIS_TRACE_UNMAPPED=1
...
uint8_t* unmapped_access(uint32_t address, uint32_t length) {
    ...
    static const bool trace = std::getenv("CUBIS_TRACE_UNMAPPED") != nullptr;
    if (trace) {
        std::fflush(stdout);
