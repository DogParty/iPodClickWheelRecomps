...
std::string default_data_directory() {
    ...
    return "sdmc:/switch/cubis";
#elif defined(__APPLE__)
    return environment("HOME") + "/Library/Application Support/iPod Cubis 2";
#elif defined(_WIN32)
    return environment("APPDATA") + "\\iPod Cubis 2";
#else
    const std::string xdg = environment("XDG_DATA_HOME");
    return (xdg.empty() ? environment("HOME") + "/.local/share" : xdg) + "/ipod-cubis";
#endif
}
...
std::string data_directory() {
    std::string directory = environment("CUBIS_DATA_DIR");
    if (directory.empty()) {
