...
// CUBIS_TRACE_INPUT=1 prints every key the window receives, with what it is bound to. What a
...
bool trace_input() {
    static const bool on = SDL_getenv("CUBIS_TRACE_INPUT") != nullptr;
    return on;
}

// CUBIS_TRACE_AUDIO=1 prints what the audio is asked to do — the track opened and its shape, the
...
bool trace_audio() {
    static const bool on = SDL_getenv("CUBIS_TRACE_AUDIO") != nullptr;
    return on;
}
