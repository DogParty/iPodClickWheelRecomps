...
struct Settings {
    // Frames a second to pace to; 0 runs as fast as the machine can. 30 is the rate the device
    // ran this game at, and the rate its own arithmetic wants: it divides by the frame time in
    // 64ths of a second, so a shorter frame is a divide by zero (PLAN.md difference 5).
