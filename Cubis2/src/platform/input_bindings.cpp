...
// The order here is the order a settings window lists them: the wheel's two gestures — turning
// it, then resting on it, clockwise from the top — then the button in the middle, then the four
// around the outside as they sit on the device.
constexpr ActionNames NAMES[ACTION_COUNT] = {
    ...
    {Action::TouchUp, "touch-up", "Touch Up"},
    {Action::TouchRight, "touch-right", "Touch Right"},
    {Action::TouchBottom, "touch-bottom", "Touch Bottom"},
    {Action::TouchLeft, "touch-left", "Touch Left"},
...
constexpr unsigned BINDINGS_FORMAT = 4;  // 4: the four Touch actions took the arrow keys
...
std::string InputBindings::to_text() const {
    ...
    out << "# Cubis 2 input bindings: one action per line, then the platform's codes for the\n"
           "# two inputs that perform it. A code of 0 means deliberately unbound.\n";
