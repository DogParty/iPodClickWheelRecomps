...
// How the microsecond clock (`device::clock_microseconds`) advances. This game reads it several
// times a frame and divides by the difference between one frame's reading and the next's, so
// the choice decides how fast the game runs (PLAN.md difference 5):
//
//   * per call — the emulator's `--fixed-clock`, 16 667 µs a call, which every recording in
//     tests/expected/ was made with. `--emulator-firmware` asks for this.
//   * per frame (the default) — the pump calls `advance_clock` once a frame with the frame's
//     length, never less than a 64th of a second, and each call within the frame reads one
//     microsecond later than the last.
