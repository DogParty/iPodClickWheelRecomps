...
// CUBIS_TRACE_FILES=1 prints one line per file operation. The emulator has the same in
...
void trace(const char* what, const std::string& name, uint32_t a, uint32_t b, uint32_t result) {
    static const bool on = std::getenv("CUBIS_TRACE_FILES") != nullptr;
    if (on) {
...
uint32_t open_for_read(const std::string& name) {
    ...
    // The bytes go to the game exactly as they ship. The Vortex recomp corrects three
    // mislabelled texture headers here (its `gamedata/asset_fixes.h`) because reading its own
    // loader proved the labels wrong; no such defect has been established in this title's data,
    // and a correction applied on anything less than that proof would be this program inventing
    // a game it is meant to be reproducing. If one is ever found, this is where it goes, and it
    // arrives with the `--original-assets` switch the oracle needs to opt out of it.
...
// **two** of its own at the root of its folder (PLAN.md difference 3): `cubissave.dat` (21 760
// bytes) and `cubisgame.dat` (9 264). Neither is a guess — the game's own `Manifest.plist` lists
// the 114 files Apple shipped and names neither of them.
//
// Both are opened *for writing with the whole file already in the buffer* and completed without a
// separate transfer, which is this title's file model and not the previous ones':
//
//   async open "cubissave.dat" mode 0x1 op 6 req … buf=0x1931a174 len=21760 … -> handle 100
//   completion req … cb 0x18034408
//
// They are read from and written to the platform's store rather than the game directory, so a
// platform that keeps saves somewhere else (a console's archive, an app's private storage) reads
// them back from there too.
bool is_save_name(const std::string& name) {
    const std::string lower = lowercase(name);
    return lower == "cubissave.dat" || lower == "cubisgame.dat";
}

// What the store keeps a file under, for every path a game saves through — and there can be two
// of those, which is why this exists rather than being inlined at one of them: a title writes
// some saves through the ordinary file calls and some through the store ordinals (#12/#14/#16),
// and both end at the same `platform::save_store()`.
//
...
// device path). The Vortex recomp found what that costs — its `en/stats` arrived as `stats` and
// *overwrote the root `stats`*, 2 128 bytes of per-language statistics on top of a 228-byte file,
// every time the game was left. Joining the last two components with a dot keeps them apart, and
// leaves a plain name (`cubissave.dat`) exactly as it is. **This title's two saves are both at
// the root**, so nothing here is load-bearing for it today; it is kept because the rule is the
// store's, not the title's, and losing it is silent.
std::string store_key(const std::string& name) {
    const std::string lower = lowercase(name);
...
uint32_t async_open(uint32_t open_mode, uint32_t path, uint32_t request) {
    ...
    // the game directory.
    //
    // Anything the store already holds is a save, whatever it is called; `is_save_name` covers
    // only the ones that must open *before* they have ever been written. Asking the store first
    // is what lets a title whose save name the player chooses work without a list — the store
    // holds exactly what this game has written, and nothing the game ships is called any of
    // those names.
    std::vector<uint8_t> saved;
    const bool stored = platform::save_store().load(store_key(name), saved);
    const bool save = stored || is_save_name(name);
    // A save is opened to be *read*, whatever mode it names — and the mode this game names is 1,
    // the same one a plain file layer would read as "write". This is measured, not assumed. Its
    // two saves are opened exactly once each per boot, with the whole file's worth of buffer
    // already attached and no transfer afterwards at all:
    //
    //   async open "cubissave.dat" mode 0x1 op 6 req … buf=0x1931a174 len=21760 … -> handle 100
    //   async open "cubisgame.dat" mode 0x1 op 6 req … buf=0x1931f674 len=9264  … -> handle 101
    //   completion req … cb 0x18034408      (both, between frames)
    //
    // — which is a *load*: an open carrying a buffer is one (`--load-on-open`), and there is no
    // second call that could be the read. Believing the mode instead would point that buffer at
    // the file and write the game's uninitialised memory over the save on every launch, which is
    // exactly what it cost The Sims Bowling before its file layer learned this rule: its `stats`
    // came back 228 bytes of zeros and the player's statistics were gone before the title screen.
    //
    // What this title has *not* shown is the write. Nothing in the recorded cases reaches a
    // point where the game saves — the oracle runs with writes refused, and a level ended by the
    // clock running out is not a level ended by the player. PLAN.md carries that.
...
namespace cubis::storage {

using namespace cubis::eapp;  // NOLINT(google-build-using-namespace): one file, by design

// Published for the tests; see framework/storage.h. The rule itself is `store_key` above.
std::string save_key(const std::string& name) {
    return store_key(name);
}
