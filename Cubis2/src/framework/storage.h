...
// The key the platform's save store keeps `name` under.
//
// Published so that a test can ask this layer where a save went rather than keeping a second
// copy of the rule — a copy is exactly how the rule stops being checked. The store's names are
// flat, so a save in a subdirectory is keyed by its last two path components joined with a dot;
// a plain name is itself, lowercased. `src/libeapp/async_file.cpp` explains what it cost to
// learn that.
[[nodiscard]] std::string save_key(const std::string& name);
