...
// `Metadata #125` — the index of the track playing now, or NO_TRACK when nothing is. This
// title asks once, at boot, while it is setting its own music up; the emulator answers "none"
// (reference/eapp-loader/lib.rs, `Stub::Value(u32::MAX)` beside the comment "current index =
// none"), which is the empty library's honest answer and what the recordings contain.
constexpr uint32_t NO_TRACK = 0xffffffffu;
[[nodiscard]] uint32_t now_playing_index();
