...
#
#   tests/frames.sh <case> [path/to/vortex-headless]
#
...
# word, and this title has more for it to judge than most: a second texture unit, the framebuffer
# copied into a texture, and the float matrix path (PLAN.md difference 6).
#
...
case_name=${1:?usage: tests/frames.sh <case> [vortex-headless]}
binary=${2:-}
...
# design decision.
VORTEX_SHOT_DIR="$frames_dir/recomp" "$binary" "$game_dir/$GAME_IMAGE_PATH" \
    --gamedir="$game_dir" --script="$script" --frames="$frames" --time=00:00 --emulator-firmware --original-assets \
    --emulator-graphics \
