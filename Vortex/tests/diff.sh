...
#
#   tests/diff.sh <case> [path/to/vortex-headless] [--lines=N] [--exact]
#
...
case_name=${1:?usage: tests/diff.sh <case> [vortex-headless] [--lines=N] [--exact]}
shift
binary=build/vortex-headless
limit=0
mode=""
for argument in "$@"; do
    case $argument in
...
# --original-assets: the recordings are of the game's files exactly as they ship, and three of
# this title's texture headers are mislabelled (src/gamedata/asset_fixes.h). A corrected run asks
# the driver for GL_ALPHA where the recording has GL_RGB, so the oracle wants the shipped bytes.
#
...
"$binary" "$image" --gamedir="$GAME_DIR" --script="$script" --call-log="$actual" --frames="$frames" --time=$clock --emulator-firmware --original-assets \
