...
using namespace vortex::platform;
...
void test_defaults() {
    ...
    // 30, not the 60 the other titles default to: this game divides by its frame time in
    // 64ths of a second and the device ran it at 30 (PLAN.md difference 5).
    check(fresh.frame_rate == 30, "the device's rate for this game is the default rate");
...
void test_persistence() {
    const std::filesystem::path directory =
        std::filesystem::temp_directory_path() / "vortex-settings-test";
