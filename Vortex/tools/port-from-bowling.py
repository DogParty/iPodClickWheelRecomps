#!/usr/bin/env python3
"""Copy the layers this title has not yet been able to take from the shared core out of the Sims
Bowling recomp, and record what was copied.
...
Sims Bowling, not Hold'em, is the source for every file: its tree is the newest copy of the same
layers — it carries the forwarding headers into the shared core, the fixes made since (the
unmapped-access model in `memory.cpp`, the seek and byte-count rules in `async_file.cpp`, the
widest tracing aids) — and where Hold'em's and Sims Bowling's copies of a file differ at all, the
difference is one of those fixes. Where Vortex's *behaviour* is Hold'em's rather than Sims
Bowling's (the button-flags word and the constant reason byte, PLAN.md differences 1 and 2), the
Hold'em file is read and the change made by hand here, with its provenance in the comment.

**Identifiers are rewritten; prose is not.** The C++ namespace and everything built on it
(`bowling` -> `vortex`), the CMake options and include guards (`BOWLING_` -> `VORTEX_`), the
target and program names and the environment variables are rewritten as the files are copied,
because a namespace called `bowling` in this tree is exactly what the quality rules exist to
prevent. A comment that says "The Sims Bowling keeps its save in `savefile.dat`" is left saying
so: it is a true statement about that game, and rewriting it to name this one would make it a
false one. Each ported file's prose is corrected by hand when the file is adopted for this title —
`grep -rn "Sims Bowling\\|Bowling\\|Hold'em\\|\\bLost\\b" src tests tools` is the worklist, and it
should reach zero as the port is finished.

    python3 tools/port-from-bowling.py                 # copy every file, write the manifest
    python3 tools/port-from-bowling.py --only PATH...  # copy just these, keeping the rest
    python3 tools/port-from-bowling.py --check         # report drift; copy nothing

`--check` re-hashes the Sims Bowling originals and this tree's copies and says which files have
...
SOURCE = PROJECT.parent / "Sims Bowling"
...
#
# Not ported, and why:
#   src/game/            Sims Bowling has none yet. Nothing to inherit; this is the work.
#   src/gamedata/manifest_data.cpp   a table of Sims Bowling's own files. `tools/manifest.py` is
#                        ported and writes this title's from its own folder.
#   tests/expected/, tests/scripts/   recordings and scripts of another game.
#   tests/exact-allow.txt   the ordinal Sims Bowling's exact comparison drops, for a reason that
#                        is that title's (a wall-clock leftover in a stack slot). Whether this
#                        title needs one is answered by its own exact comparison.
#   tools/port-from-holdem.py   this file's ancestor; this file replaces it.
...
# `bowling_foo` and the two prefixed rules have to come first.
REWRITES = [
    (r"\bBOWLING_", "VORTEX_"),
    (r"\bbowling_", "vortex_"),  # CMake targets and C++ symbols
    (r"\bBOWLING\b", "VORTEX"),
    (r"\bbowling\b", "vortex"),  # the namespace, the program name, the Linux data directory
    (r"iPod The Sims Bowling", "iPod Vortex"),  # the per-user data directory (paths.cpp)
    (r"ipod-sims-bowling", "ipod-vortex"),
]
...
def check() -> int:
    ...
    for title, paths in (
        ("changed in Sims Bowling since the port", drifted_source),
